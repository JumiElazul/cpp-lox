...
logger::logger()
{
    ...
    _logger->set_pattern("[%Y-%m-%d %H:%M:%S] [%^%l%$] [%s:%# : func<%!>] %v");
...
{
    log_trace("GEO_TRACE level enabled");
    log_debug("GEO_DEBUG level enabled");
    log_info("GEO_INFO level enabled");
    log_warn("GEO_WARN level enabled");
    log_error("GEO_ERROR level enabled");
    log_critical("GEO_CRITICAL level enabled");
}
