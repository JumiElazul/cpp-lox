option(CPPLOX_ENABLE_CONSOLE_LOGGING "Enable logging to the console" ON)
option(CPPLOX_ENABLE_FILE_LOGGING "Enable logging to the cpp-lox.log file" ON)

set(LOG_LEVELS TRACE DEBUG INFO WARN ERROR CRITICAL OFF)

set(CPPLOX_CONSOLE_LOG_LEVEL INFO CACHE STRING "Log level for console. Options: ${LOG_LEVELS}")
set(CPPLOX_FILE_LOG_LEVEL TRACE CACHE STRING "Log level for file. Options: ${LOG_LEVELS}")

foreach(var CPPLOX_CONSOLE_LOG_LEVEL CPPLOX_FILE_LOG_LEVEL)
...
target_compile_definitions(cpp-lox-core PUBLIC SPDLOG_ACTIVE_LEVEL=SPDLOG_LEVEL_TRACE)
...
function(set_spdlog_levels target console_level file_level)
    ...
    if (CPPLOX_ENABLE_CONSOLE_LOGGING)
        target_compile_definitions(${target} PUBLIC 
            CPPLOX_ENABLE_CONSOLE_LOGGING
            CPPLOX_CONSOLE_LOG_LEVEL=${console_value})
    endif()

    if (CPPLOX_ENABLE_FILE_LOGGING)
        target_compile_definitions(${target} PUBLIC
            CPPLOX_ENABLE_FILE_LOGGING
            CPPLOX_FILE_LOG_LEVEL=${file_value})
    endif()
endfunction()

set_spdlog_levels(cpp-lox-core ${CPPLOX_CONSOLE_LOG_LEVEL} ${CPPLOX_FILE_LOG_LEVEL})

if (CPPLOX_ENABLE_CONSOLE_LOGGING)
    message(STATUS "Console logging enabled at level: ${CPPLOX_CONSOLE_LOG_LEVEL}")
else()
    message(STATUS "Console logging disabled")
endif()
if (CPPLOX_ENABLE_FILE_LOGGING)
    message(STATUS "File logging enabled at level: ${CPPLOX_FILE_LOG_LEVEL}")
else()
